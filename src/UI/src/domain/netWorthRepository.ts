...
export type NetWorthConfiguration = { trackMortgageInNetWorth?: boolean; netWorthGoal?: number };
...
export type NetWorthRepository = {
  ...
  putConfiguration?: (configuration: NetWorthConfiguration | boolean) => Promise<NetWorth>;
...
export const createMockNetWorthRepository = (initialValue?: number): NetWorthRepository => {
  let value: NetWorth | undefined = { beginningNetWorth: initialValue ?? 100000, monthlyAccountValues: {}, trackMortgageInNetWorth: true, netWorthGoal: 0, mortgageSchedule: { houseValue: 800000, startingOutstandingMortgage: 0, annualInterestRate: 0.02875, monthlyPrincipalPayment: 0, monthlyAdditionalPrincipalPayment: 0, scheduleStartMonth: '2026-01' }, updatedAt: '2026-01-01T00:00:00Z' };
  const merge = (next: Partial<NetWorth>): NetWorth => value = { beginningNetWorth: value?.beginningNetWorth ?? 100000, monthlyAccountValues: value?.monthlyAccountValues ?? {}, trackMortgageInNetWorth: value?.trackMortgageInNetWorth ?? true, netWorthGoal: value?.netWorthGoal ?? 0, mortgageSchedule: value?.mortgageSchedule ?? null, updatedAt: '2026-01-01T00:00:00Z', ...next };
  return {
    get: async () => value,
    ...
    putConfiguration: async (configuration) => merge(typeof configuration === 'boolean' ? { trackMortgageInNetWorth: configuration } : configuration),
