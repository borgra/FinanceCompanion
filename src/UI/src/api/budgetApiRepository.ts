...
export const createBudgetApiRepository = (client: HttpClient): BudgetRepository => ({
  ...
  createCategory: (name: string, colorHex: string) =>
    client.post<BudgetCategory>('/budget/categories', { name, colorHex }),
  updateCategory: (id: string, name: string, colorHex: string) =>
    client.put<BudgetCategory>(`/budget/categories/${id}`, { name, colorHex }),
