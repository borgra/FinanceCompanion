...
from app.domain.dividend_research import DividendResearchValidationError
...
def delete_mortgage_schedule(user=Depends(require_session_user), container=Depends(get_container)) -> NetWorthPayload:
    ...
    value = container.put_net_worth.execute(user.user_id, replace(current, mortgage_schedule=cleared_schedule, updated_at=now_iso()))
    return _to_net_worth_payload(value)
    ...
    updated, unmatched = container.import_holding_details.execute(
        ...
        [
            (
                ...
                "dividend_growth_rate" in row.model_fields_set,
                row.dividend_growth_rate,
            )
            for row in request.rows
        ],
    )
...
@router.post("/holdings/{holding_id}/dividends/refresh", response_model=HoldingPayload)
def refresh_holding_dividends(
    holding_id: str,
    user=Depends(require_session_user),
    container=Depends(get_container),
) -> HoldingPayload:
    try:
        return to_holding_payload(
            container.refresh_holding_dividends.execute(user.user_id, holding_id)
        )
    except DividendResearchValidationError as exc:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Dividend research returned an invalid result.",
        ) from exc
    except DomainError as exc:
        raise _domain_error_to_http(exc) from exc
