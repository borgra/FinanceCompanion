...
def seed_cosmos_database(client: TableClient, allowed_email: str) -> None:
    """Seeds the Cosmos Table with the initial layout if the allowed user profile does not exist."""
    allowed_email_cf = allowed_email.casefold()
    
    # Check if mappings exist
    try:
        client.get_entity("users_by_email", allowed_email_cf)
        # Already seeded
        return
    except ResourceNotFoundError:
        pass

    # Clean seed data cloning
    seed_data = clone_seed_data(allowed_email=allowed_email)
    
    # 1. Seed Users and mappings
    for raw_user in seed_data["users"]:
        user = User(
            id=raw_user["id"],
            email=raw_user["email"],
            display_name=raw_user["displayName"],
            identity_subject=raw_user.get("identitySubject"),
            identity_object_id=raw_user.get("identityObjectId"),
            identity_tenant_id=raw_user.get("identityTenantId"),
            picture_url=raw_user.get("pictureUrl"),
        )
        
        # User details profile row
        client.upsert_entity(_user_to_entity(user))
        
        # Secondary mapping row
        client.upsert_entity({
            "PartitionKey": "users_by_email",
            "RowKey": user.email.casefold(),
            "userId": user.id,
        })
        
    # 2. Seed Income Sources
    for user_id, items in seed_data["income_sources"].items():
        for raw_src in items:
            periods = [
                IncomePeriod(
                    id=p["id"],
                    start_date=p["startDate"],
                    yearly_gross_amount=int(p["yearlyGrossAmount"]),
                    net_percentage=int(p["netPercentage"]),
                    end_date=p.get("endDate"),
                )
                for p in raw_src["periods"]
            ]
            source = IncomeSource(
                id=raw_src["id"],
                name=raw_src["name"],
                type=raw_src["type"],
                cadence=raw_src["cadence"],
                periods=periods,
                status=raw_src["status"],
                created_at=raw_src["createdAt"],
                updated_at=raw_src["updatedAt"],
            )
            client.upsert_entity(_income_source_to_entity(user_id, source))
            
    # 3. Seed Budget Categories
    for user_id, items in seed_data["budget_categories"].items():
        for raw_cat in items:
            sub_categories = [
                BudgetSubCategory(
                    id=s["id"],
                    category_id=s["categoryId"],
                    name=s["name"],
                    monthly_amount_usd=int(s["monthlyAmountUsd"]),
                    created_at=s["createdAt"],
                    updated_at=s["updatedAt"],
                )
                for s in raw_cat.get("subCategories", [])
            ]
            category = BudgetCategory(
                id=raw_cat["id"],
                name=raw_cat["name"],
                color_hex=raw_cat["colorHex"],
                created_at=raw_cat["createdAt"],
                updated_at=raw_cat["updatedAt"],
                sub_categories=sub_categories,
            )
            client.upsert_entity(_budget_category_to_entity(user_id, category))

    # 4. Seed Accounts
    for user_id, items in seed_data["accounts"].items():
        for raw_acc in items:
            columns = [
                AccountColumn(
                    id=c["id"],
                    name=c["name"],
